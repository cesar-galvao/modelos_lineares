...
\frac{1}{\sqrt{x_2^p}} & \frac{x_2}{\sqrt{x_1^p}} \\
\vdots & \vdots \\
\frac{1}{\sqrt{x_2^p}} & \frac{x_n}{\sqrt{x_1^p}}
\end{bmatrix}
$$
...
```{r}
fit_w1 <- lm(ntrabalhadores ~ nsupervisores, weights = 1/(ntrabalhadores)^1, data = supervisores)
fit_w2 <- lm(ntrabalhadores ~ nsupervisores, weights = 1/(ntrabalhadores)^2, data = supervisores)
fit_w3 <- lm(ntrabalhadores ~ nsupervisores, weights = 1/(ntrabalhadores)^3, data = supervisores)
...
De fato, observa-se que o modelo transformado explica em torno de 20% a mais de variância do que o modelo não transformado, para qualquer valor de $p$ dentre os disponíveis.
...
Quanto à avaliação da normalidade, utilizando os gráficos de envelope simulado a seguir, $p=3$ parece resultar na pior opção, enquanto $p=2$ parece resultar na melhor opção.
...
Considerando as possiblidades apontadas, opta-se pelo modelo em que $p=2$.
