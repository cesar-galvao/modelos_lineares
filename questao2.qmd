...
```{r}
# X <- matrix(c(rep(1, 27),supervisores$ntrabalhadores), ncol = 2)
# V_1 <- solve(diag(supervisores$ntrabalhadores^3))
# 
# P_1 <- diag(1/sqrt(supervisores$ntrabalhadores^3))
# 
# Q <- P_1 %*% X
# 
# Y <- supervisores$nsupervisores
# 
# betaG <- solve(t(X)%*%V_1%*%X) %*% t(X) %*% V_1 %*% supervisores$nsupervisores
# 
# xtxInv <- solve(t(X) %*% V_1 %*% X)
# 
# Zhat <- fitted(fit_w3)
# Z <- P_1 %*% Y
# 
# sqres <- sum((Z[,1]-Zhat)^2) #????
# 
# (confint(fit_w2, level = .9) %>% as_tibble())[2,]
# 
# t(Y)%*%V_1%*%Y
```
